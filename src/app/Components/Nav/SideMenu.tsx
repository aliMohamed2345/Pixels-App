          ...
          {sideMenuOptions.map((option, i) => (
            <li key={i}>
              <Link
                title={option.name}
                href={`/${
                  option.name.includes(`Home`) ? "" : option.name.toLowerCase()
                }`}
                className={`flex items-center justify-start gap-5 rounded-2xl p-2 font-bold hover:bg-amber-300 dark:hover:text-white transition-all mx-4 text-lg text-text_color ${
                  // pathname.includes(option.name.toLowerCase()) ||
                  (pathname === "/" && option.name.trim() === "Home") ||
                  (pathname.includes(option.name.toLowerCase()) &&
