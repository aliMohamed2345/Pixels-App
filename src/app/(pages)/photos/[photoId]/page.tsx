...
const PhotoId = () => {
  const { imageId, tags, type, width, height, src, alt, Favorite } =
  ...
  function handleFavoriteBtn(id: number) {
    ...
    if (isFavorite) {
      ...
      const updatedPhotos = previousPhotos.filter(
        (photo) => String(photo.imageId) !== String(id)
      );
  ...
  useEffect(() => {
    const filterData = () => {
      const apiUrl = `${process.env.NEXT_PUBLIC_PHOTOS_API}?key=${process.env.NEXT_PUBLIC_PHOTOS_API_KEY}`;
      ...
      fetch(filteredLink)
        ...
        .catch((error) => {
          // Handle errors, such as invalid response or fetch failure
        ...
        <div className="shadow-lg rounded-lg p-5 bg-secondary text-text_color flex flex-col gap-5 w-[65vw] text-center  ">
          ...
          <div className="gap-2 flex items-center flex-wrap justify-center">
            {uniqueTags.map((tag, i) => (
              <Link
                href={{ pathname: `/search/photos/${tag}`, query: { q: tag } }}
          ...
          <div className="flex gap-3 items-center justify-center">
            <button
              onClick={() =>
                handleDownload(src, `${uniqueTags[0]} image from pixels`)
              }
              className="rounded-full flex-1 p-3 bg-green-500 text-white text-center transition-all font-bold hover:scale-105"
            >
              download
            </button>
            <button
              onClick={() => {
                handleFavoriteBtn(+imageId);
              }}
              type="button"
              title="favorite"
